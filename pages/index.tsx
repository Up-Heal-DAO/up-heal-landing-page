...
const Home: NextPage = () => {
  ...
  return (
    <Box bgGradient={'linear-gradient(0deg, brand.100 51.22%, rgba(255, 182, 193, 0.5) 114.97%)'}>
      <Head>
      ...
      <Box position='relative' overflow='hidden' pt={{ base: '58px', md: 40 }} pb={{ base: '98px', md: '137px' }}>
        ...
        {isMobile
          ? <Image src='images/line-2-mobile.svg' alt='' position='absolute' w={{ base: '10%' }} h='auto' right='0' top='0' transform='auto' translateY='-22%' />
          : <Image src='/images/line-2.svg' alt='' position='absolute' w='auto' h='100%' right='0' top='-87px' transform='auto' translateX='18%' />
        }
        <Box maxWidth='735px' mx='auto'>
          <Heading fontSize={{ base: '40px', md: '54px' }} textAlign={'center'} color={'white'}>
            Up Heal DAO
          </Heading>
          <Text fontSize={{ base: '16px', md: '32px' }} color={'white'} textAlign="center" mt={4} maxWidth={{ base: '296px', md: '735px' }} mx='auto'>We are funding Mental Health Organizations and community initiatives.</Text>
          <Flex justifyContent={'center'}>
            <Link display='block' px={{ base: 2.5, md: 6 }} py={{ base: 2, md: 4 }} fontSize={{ base: 'xs', md: 'lg' }} mt={{ base: 10, md: 14 }} color={'brand.100'} borderRadius='full' bg={'white'} href='https://foul-color-34f.notion.site/Up-Heal-DAO-8c250e000a874920ad60bd23105155e0' isExternal>
              Get Started
            </Link>
          </Flex>
        </Box>
      </Box>
      <Box display={{ md: 'flex' }} maxWidth={'container.xl'} mx='auto' justifyContent='center' pb={20}>
        <Flex justifyContent='center' pl={10}>
          <Text lineHeight='shorter' fontSize={{ base: '40px', md: '80px' }} color={'white'} fontWeight={700} textTransform='uppercase' width='min-content'>
            Who We are
          </Text>
          <Image src='/images/line-3.png' alt='' display={{ md: 'none', xl: 'block' }} maxHeight={{ base: '148px', md: '258px' }} mt={{ base: -2.5, md: 0 }} />
        </Flex>
